...
class SearchHandler(BaseHandler.BaseHandler):
  ...
  def post(self):
    ...
    if gender == "male":
      qry = ndb.gql("SELECT * FROM Record WHERE estce_que_votre_organisation_offre_des_services_en_sant_men = 'oui'")
    if gender == "female":
      qry = ndb.gql("SELECT * FROM Record WHERE estce_que_votre_organisation_offre_des_services_en_sant_men = 'non'")
